...
- `category()` accessors now accept the key with its `${category}.` prefix stripped (e.g. `'currency'` instead of `'billing.currency'`) on `get`/`set`/`unset`/`history`, in addition to the full key — both forms address the same setting. If a given string already matches a real, existing key, it's used as-is rather than guessed at; only unmatched strings are treated as short and prefixed.
